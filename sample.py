...
def select_accounts(yb):
    ...
    with yb.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as yb_cursor:
        ...
        for row in results:
            print(
                "name = {name}, age = {age}, country = {country}, balance = {balance}".format(
                    **row
                )
            )
...
def transfer_money_between_accounts(yb, amount):
    try:
        with yb.cursor() as yb_cursor:
            yb_cursor.execute(
                "UPDATE DemoAccount SET balance = balance - %s WHERE name = 'Jessica'",
                [amount],
            )
            yb_cursor.execute(
                "UPDATE DemoAccount SET balance = balance + %s WHERE name = 'John'",
                [amount],
            )
        yb.commit()
